...
### 6.1 previewing photo

- 파일 이벤트의 경우, `event.target.value`를 사용하면 콘솔에 파일 경로가 출력되지만, 브라우저 보안 정책 상 실제 경로를 표시하면 안 되므로 상세 경로가 숨겨진 상태로 출력된다. 따라서 `event.target.files`로 파일 관련 내용을 확인한다. 여기서 files는 파일 배열이다. input 엘리먼트에 `multiple` 속성을 추가하면, 여러 개의 파일을 한 번에 선택할 수 있다. 그러면 여러 파일의 정보가 배열로 나타나게 된다.
- 첨부파일의 종류를 제한하고 싶다면 input 태그에 accpet 속성을 추가한다. 사진만 첨부하게 만들려면 `accept="image/*`와 같이 입력한다.
- 웹 브라우저에 사진 출력하는 브라우저 API : `FileReader`
  - `new`와 함께 사용해야 한다.
  - `readAsDataURL` 함수는 파일 정보를 인자로 받아서 파일 위치를 URL로 반환한다. 하지만 이 함수는 단순히 호출하는 방식으로 사용할 수 없다. 리액트 생명주기 함수처럼 파일 선택 후 '웹 브라우저가 파일을 인식하는 시점', '웹 브라우저 파일 인식이 끝난 시점'등을 포함하고 있어서 시점까지 함께 관리해줘야 URL을 얻을 수 있다.
  - `onloadend` : `readAsDataURL` 함수에 전달할 인자, 즉, 파일이 함수로 들어간 이후 결괏값이 나온 다음 상황을 감지한다. 그때 생긴 이벤트값을 사용할 수 있게 해준다.

### 6.2 saving photo

- firestore은 파일 저장에 적합하지 않기 때문에, 사진, 동영상 같은 파일을 저장하려면 파이어베이스 스토리지를 사용해야 한다. 파이어베이스 스토리지는 사진, 동영상 같은 덩치가 큰 파일을 저장했다가 필요할 때 꺼내서 사용할 수 있게 해주는 곳이다.
- storage
  - firestore와의 차이점 : storage는 문서에 아이디를 자동으로 만들어 주지 않는다. 따라서 개발자가 이 부분을 직접 구현해야 한다. 즉, 고유 식별자를 만들어서 스토리지에 저장할 데이터에 짝을 지어줘야 한다. -> UUID(Universaly Unique Identifier) 라이브러리 사용한다. [깃허브 UUID](https://github.com/uuidjs/uuid)
  - 레퍼런스라고 하는 경로 시스템을 가지고 있다. 레퍼런스를 통해 경로를 생성하면 이 경로를 이용해서 파일을 업로드하거나 지우는 등의 작업을 할 수 있다. 파이어스토어는 '컬렉선/문서 아이디'와 같이 경로를 표현하였다. 스토리지의 레퍼런스에도 아이디가 필요하지만, 자동으로 생성해주지 않으므로 직접 아이디를 만들어주어야 한다.
  - 스토리지에 사용자 아이디로 폴더를 만들고, 폴더 안에 UUID로 파일 이름을 정해서 저장한다. 이 작업은 모두 리퍼런스 함수를 통해서 한다. -> `ref().child()` : 폴더, 파일 이름을 설정할 수 있다.

#### 공식문서

- [자바스크립트 스코프](https://ko.javascript.info/closure)
